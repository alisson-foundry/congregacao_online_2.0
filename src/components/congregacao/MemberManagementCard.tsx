...
// Helper function to get permission badges
...
export interface MemberManagementCardProps {
  members: Membro[];
  onAddMember: () => void;
  ...
  handleOpenAdvancedOptions: (memberId: string | null) => void;
  className?: string;
}
...
export function MemberManagementCard({
  ...
  return (
    <Card className={cn("bg-white text-black border-none shadow-none", className)}>
      <CardHeader>
                ...
                {members.map((member) => (
                  <TableRow key={member.id}>
                    <TableCell className="font-medium">
                      <button onClick={() => onEditMember(member)} className="hover:underline text-blue-600">
                        {member.nome}
                      </button>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => onDeleteMember(member.id)} className="text-red-600 focus:text-red-600 focus:bg-red-100">
