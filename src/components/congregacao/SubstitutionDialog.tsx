...
export function SubstitutionDialog({
    ...
    try {
      ...
      if (candidate) {
        ...
        toast({ 
          ...
          description: "Não foi encontrado um candidato automático elegível.", 
          variant: "default" 
        });
      }
    } catch (e: any) {
  ...
  const handlePrepareManualSelection = async () => {
    ...
    if (!targetFunction) {
      const msg = `Definição da função "${resolvedFunctionId}" não encontrada.`;
      setError(msg);
      toast({ 
        title: "Erro de Configuração", 
        description: "Função inválida para substituição.", 
        variant: "destructive" 
      });
      setIsLoadingManual(false);
      return;
    ...
    try {
      ...
      if (substitutes.length > 0) {
        ...
        allMembers.forEach(membro => {
          if (membro.id === originalMemberId) return;
        ...
        toast({ 
          title: "Nenhum Substituto", 
          description: "Verifique os detalhes no diálogo.", 
          variant: "default" 
        });
      }
    } catch (e: any) {
  ...
  const getFunctionGroupName = (groupId: string) => {
    switch (groupId) {
      case 'Indicadores':
        return 'Indicadores';
      case 'Volantes':
        return 'Volantes';
      case 'LeitorPresidente':
        return 'Leitor/Presidente';
      default:
        return groupId;
    }
  };
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            Função: {getFunctionGroupName(currentFunctionGroupId)} - {displayedFunctionName}
          </DialogDescription>
        </DialogHeader>
        ...
        {mode === 'options' && (
          <div className="py-4 space-y-3">
            ...
            <Button 
              onClick={handleFindAutomaticSubstitute} 
              className="w-full" 
              variant="default" 
              disabled={isLoadingAutomatic || !targetFunction}
            >
          ...
          <div className="py-4 space-y-4">
            ...
            {potentialSubstitutes.length > 0 ? (
              <ScrollArea className="h-[200px] w-full rounded-md border p-2">
                <RadioGroup 
                  value={selectedManualSubstitute || ''} 
                  onValueChange={setSelectedManualSubstitute}
                >
              ...
              <p className="text-sm text-muted-foreground">
                Nenhum membro elegível encontrado (após aplicação de todos os filtros). 
                Verifique a mensagem de erro acima para detalhes.
              </p>
            )}
            <div className="flex justify-end space-x-2">
              ...
              <Button 
                ...
                disabled={!selectedManualSubstitute}
              >
                Confirmar Seleção
              </Button>
            </div>
